...
test("walker is deterministic and ignores symlinks and agent worktrees", async () => { const root = await mkdtemp(path.join(tmpdir(), "shiplayer-walk-")); await writeFile(path.join(root, "z.swift"), "z"); await writeFile(path.join(root, "a.swift"), "a"); await mkdir(path.join(root, ".claude", "worktrees"), { recursive: true }); await writeFile(path.join(root, ".claude", "worktrees", "stale.swift"), "stale"); await symlink(path.join(root, "a.swift"), path.join(root, "linked.swift")); const first = await walkRepository(root); const second = await walkRepository(root); assert.deepEqual(first.files, second.files); assert.equal(first.symlinksIgnored.length, 1); assert.ok(first.ignoredDirectories.includes(".claude")); assert.ok(!first.files.some((file) => file.includes("stale.swift"))); });
