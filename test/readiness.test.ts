...
test("scanner evidence cannot be silently omitted from a ready manifest", async () => {
  ...
  manifest.sourceContradictionOverrides = [{ finding: "ai-sharing.source-contradiction:endpoint:https://api.openai.com/v1/models", reason: "Fixture: this call only lists model metadata and sends no user data.", evidence: ["App.swift"], confirmation: "confirmed" }];
