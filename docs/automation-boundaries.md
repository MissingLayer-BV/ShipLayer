...
| Subscriptions/IAP | Block readiness without production `Product.displayPrice` evidence, disabled-until-loaded behavior, and a visible-price UI/snapshot test; generate checklist/review notes; human verifies StoreKit/App Store Connect state and sandbox behavior. |
| Third-party AI | Block readiness without exact data/recipient/purpose disclosure before transmission, explicit permission, a non-AI decline path, and matching privacy-policy evidence; human confirms legal truth and processor protection. |
