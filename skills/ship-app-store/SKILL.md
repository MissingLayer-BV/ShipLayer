...
- `monetization` and `aiDataSharing` are cross-checked against the scan, not trusted at face value. If `analyze`/`check` shows StoreKit purchase evidence (`Product.purchase()`, `.displayPrice`/`ProductView`, a `.storekit` product ID) while `monetization.type` is `free`, or shows a call shaped like a third-party AI/inference endpoint (a known AI-provider host, or a `/chat/completions`, `/v1/messages`, `/generate`, `/inference`-style path) while `aiDataSharing.enabled` is `false`, `check` blocks loudly with `monetization.source-contradiction` or `ai-sharing.source-contradiction`. Never resolve one of these by leaving the manifest blank or guessing at the missing fields — either help the user declare the real IAP/subscription model or AI data-sharing disclosure, or, only when the user confirms the finding is not real (e.g. dead code, a docs-only link, sandbox-only scaffolding), add a `sourceContradictionOverrides` entry naming the exact blocker id with a human-authored `reason`, real `evidence` paths, and `confirmation: confirmed`. Never fabricate that reason yourself or set `confirmation: confirmed` without the user's explicit say-so — an override is a human decision, not an heuristic one. A bare privacy/docs link on a known AI-provider host with no API-shaped path only warns (`ai-sharing.possible-processor-link`); no override is needed to clear a warning.
