...
export async function generateReleasePackage(repository: string, manifest: ShipLayerManifest, analysis: AnalysisReport, preflight: PreflightReport, outputDirectory: string): Promise<PreparedPackage> {
  ...
  for (const [locale, copy] of Object.entries(manifest.metadata.localizations)) await emit(`metadata/${locale}.json`, stableJson(localeMetadataJson(locale, copy, manifest.app.releaseKind, manifest.metadata.whatsNewConfirmation)));
...
async function privacyDraft(repository: string, manifest: ShipLayerManifest, analysis: AnalysisReport, preflight: PreflightReport): Promise<string> {
  ...
  const unresolvedPermissionFlows = analysis.findings.filter((finding) => finding.key.startsWith("permissionFlow:") && !manifest.permissionFlows.some((item) => finding.key === `permissionFlow:${item.category}`));
  lines.push("", "## Permission-flow declarations (App Review 5.1.1(iv): no dismissible screen before the system prompt)");
...
export async function appReviewNotes(repository: string, manifest: ShipLayerManifest, analysis: AnalysisReport): Promise<string> {
  ...
  // An owner-decided consent flow is described in the owner's words, never as a consent screen that does not exist.
  const aiConsentOverride = manifest.aiDataSharing.enabled ? await resolveContradictionOverride(repository, manifest, AI_CONSENT_FLOW_FINDING, manifest.aiDataSharing.consent.evidence) : undefined;
  const lines = [`# ${manifest.app.name || "App"} — App Review Notes`, "", "> Draft only. Verify every statement and enter actual demo credentials in App Store Connect's secure review fields, not this file.", "", "## Access", manifest.review.demoAccount?.required ? `A demo account is required. Configure the reviewer username/password in App Store Connect secure fields. ${manifest.review.demoAccount.setupInstructions || "Setup instructions are missing."}` : manifest.review.demoAccount?.setupInstructions?.trim() ? `No demo account is supplied. ${manifest.review.demoAccount.setupInstructions.trim()}` : "No account, registration, or login is required.", "", "## Setup", ...(manifest.review.sampleData?.length ? manifest.review.sampleData.map((sample) => `- ${sample}`) : ["- Use the normal first-run flow. Add sample content as described in the recording script."]), "", "## External services", ...externalServicesLines, ...(manifest.aiDataSharing.enabled ? ["", "## Third-party AI consent", aiConsentOverride ? `The app sends ${manifest.aiDataSharing.dataSent.join("; ")} to ${manifest.aiDataSharing.processorNames.join(", ")}; purpose: ${manifest.aiDataSharing.purpose}. ${aiConsentOverride.reason}` : `Before transmission the app states that it sends ${manifest.aiDataSharing.dataSent.join("; ")} to ${manifest.aiDataSharing.processorNames.join(", ")} for ${manifest.aiDataSharing.purpose}. The affirmative action is “${manifest.aiDataSharing.consent.affirmativeAction}”; the non-AI path is ${manifest.aiDataSharing.consent.declinePath}.`] : []), "", "## Monetization", monetizationLine, "", "## Recording scenarios", ...manifest.review.recordingScenarios.map((scenario, index) => `${index + 1}. ${scenario.title}: ${scenario.steps.join(" → ")}`)];
...
function localeMetadataJson(locale: string, copy: LocaleCopy, releaseKind: ShipLayerManifest["app"]["releaseKind"], whatsNewConfirmation: ShipLayerManifest["metadata"]["whatsNewConfirmation"]): object {
  ...
  return { locale, releaseKind: releaseKind ?? "needs-human-confirmation", whatsNewRequired: releaseKind === "update", whatsNewConfirmation: whatsNewConfirmation ?? { version: "", confirmation: "needs-human-confirmation" }, ...copy, confirmation: copy.confirmation ?? "needs-human-confirmation", characterLimits, characterCounts };
}
...
function screenshotCaptureWorkflow(manifest: ShipLayerManifest, harness: { sourceFiles: string[] }, xcodeGenSpecs: string[]): string {
  ...
  const lines: string[] = [
    ...
    "            echo \"::error::No screenshot image attachments were extracted; the raw .xcresult is uploaded below for diagnosis.\"",
    "            exit 1",
    "          fi",
    "",
...
function dryRunPlan(manifest: ShipLayerManifest): string { return `# App Store Connect dry-run plan\n\nShipLayer has not made any remote changes. With App Store Connect credentials configured, run \`shiplayer plan <repo> --remote\` to compare:\n\n- App identity and version metadata for \`${manifest.app.bundleId || "MISSING_BUNDLE_ID"}\`\n- Localizations, build availability, screenshot sets, and App Review details\n- In-App Purchase/subscription state\n\nGeneration is never authorization to write. After reviewing that plan, an explicit apply may synchronize supported metadata/build/screenshots only when the user says yes, the manifest uses \`sync.mode: apply\`, and the apply confirmation flags are supplied. Manual-only or human-confirmed steps remain: initial app record creation, pricing/territories, IAP/subscription setup, privacy/age-rating/content-rights/legal/trader declarations, agreements, tax/banking, and final submission.\n`; }
function humanActions(report: PreflightReport): string { const actions = [...report.results.filter((result) => result.severity !== "pass").map((result) => `${result.message}${result.remediation ? ` — ${result.remediation}` : ""}`), "Verify/create the App Store Connect app record and enter its ID in the manifest.", "Review metadata, URLs, contacts, copyright, categories, release mode, and selected build; either explicitly authorize ShipLayer apply after its remote preview or enter them manually.", "Review actual App Store screenshots for every supported device family; either explicitly authorize ShipLayer apply after its remote preview or upload them manually.", "Configure and verify pricing and territory availability manually in App Store Connect.", "Confirm Apple Silicon Mac and Apple Vision Pro availability/compatibility, or leave each disabled after testing the chosen distribution.", "Complete App Privacy, age rating, content-rights, trader, agreement, tax/banking, and legal declarations in App Store Connect.", "Verify the public Support URL contains usable contact information.", "Perform all IAP/subscription pricing, availability, localization, review-asset, paywall, restore, and disclosure setup manually where applicable; attach a first IAP/subscription to the app-version review submission.", "Make the final review submission/release decision in App Store Connect; ShipLayer v0.1 never submits it."]; return `# Remaining human actions\n\n${[...new Set(actions)].sort().map((action) => `- [ ] ${action}`).join("\n")}\n\nNever treat generated privacy/legal text as legal advice. Confirm every declaration in App Store Connect.\n`; }
