...
import { applyGooglePlayChanges, planGooglePlayChanges } from "./play.js";
import { parsePlayScope, readPlayManifest } from "./play-manifest.js";
...
async function main(args: string[]): Promise<void> {
  ...
  if (!new Set(["init", "analyze", "prepare", "check", "plan", "capture", "apply", "submit", "draft-descriptions", "draft-listing", "draft-screenshots", "play-plan", "play-apply"]).has(command)) throw new Error(`Unknown command '${command}'.\n\n${helpText()}`);
  ...
  if (command === "play-plan" || command === "play-apply") {
    const manifest = await readPlayManifest(repository); const scope = parsePlayScope(optionValue(options, "--scope"));
    const changePlan = await planGooglePlayChanges(repository, manifest, scope);
    if (command === "play-plan" || !options.includes("--apply")) {
      output(changePlan, json, `Google Play remote preview (${scope})\n\n${changePlan.operations.map((item) => `- [${item.status}] ${item.action} ${item.resource}: ${item.description}`).join("\n")}\n\n${changePlan.warnings.map((warning) => `! ${warning}`).join("\n")}\n\nNo Google Play edit was committed.`);
      if (!changePlan.credentialsPresent) process.exitCode = 2;
      return;
    }
    if (!options.includes("--yes-i-understand")) throw new Error("Google Play mutation gate not satisfied. Pass --apply --yes-i-understand only after reviewing play-plan.");
    const result = await applyGooglePlayChanges(repository, manifest, scope, { userConfirmed: true, reviewedPlan: changePlan });
    output({ plan: changePlan, result }, json, `Google Play synchronization completed.\n\n${result.operations.map((item) => `- [${item.status}] ${item.action} ${item.resource}: ${item.description}`).join("\n")}\n\n${result.warnings.map((warning) => `! ${warning}`).join("\n")}`);
    return;
  }
...
const VALUE_OPTIONS: Record<string, Set<string>> = { prepare: new Set(["--out"]), capture: new Set(["--from", "--family", "--locale"]), "draft-listing": new Set(["--locales"]), "draft-screenshots": new Set(["--locales"]), "play-plan": new Set(["--scope"]), "play-apply": new Set(["--scope"]) };
function validateOptions(command: string, options: string[]): void {
  const allowed: Record<string, Set<string>> = { "draft-screenshots": new Set(["--json", "--apply", "--yes-i-understand", "--locales", "--replace-incomplete"]), "draft-descriptions": new Set(["--json", "--apply", "--yes-i-understand"]), "draft-listing": new Set(["--json", "--apply", "--yes-i-understand", "--locales"]), "play-plan": new Set(["--json", "--scope"]), "play-apply": new Set(["--json", "--scope", "--apply", "--yes-i-understand"]), init: new Set(["--force", "--json"]), analyze: new Set(["--json"]), prepare: new Set(["--out", "--json"]), check: new Set(["--json", "--remote"]), plan: new Set(["--json", "--remote"]), capture: new Set(["--json", "--execute", "--yes-execute", "--from", "--family", "--locale"]), apply: new Set(["--json", "--apply", "--yes-i-understand"]), submit: new Set(["--json", "--submit", "--yes-submit"]) };
