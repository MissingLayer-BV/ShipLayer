...
async function plan(repository: string, json: boolean, remote: boolean): Promise<void> { const plan = await appStorePlan(await readManifest(repository), remote); output(plan, json, `${plan.mode === "remote" ? "Remote read/discovery plan" : "Offline dry-run plan"}\n\n${plan.operations.map((item) => `- [${item.safety}] ${item.action} ${item.resource}: ${item.description} (${item.status})`).join("\n")}\n\n${plan.warnings.map((warning) => `! ${warning}`).join("\n")}`); if (remote && !plan.credentialsPresent) process.exitCode = 2; }
async function capture(repository: string, options: string[], json: boolean): Promise<void> { const plan = await createCapturePlan(repository, await readManifest(repository)); if (options.includes("--execute")) { if (!options.includes("--yes-execute")) throw new Error("Refusing capture execution without --yes-execute. Run without --execute to inspect the missing harness requirements."); const results = await executeCapturePlan(plan); output({ plan, results }, json, "No generic capture command was run."); return; } output(plan, json, `${plan.prerequisites.map((item) => `- ${item}`).join("\n")}\n\nShipLayer v0.1 outputs a deterministic harness hand-off; it does not fabricate or execute screenshot capture.`); }
async function apply(repository: string, options: string[], json: boolean): Promise<void> { const manifest = await readManifest(repository); const report = await preflight(repository, manifest, false); const changePlan = await appStorePlan(manifest, false); if (!options.includes("--apply")) { output({ mode: "dry-run", report, plan: changePlan }, json, "DRY RUN: ShipLayer made no App Store Connect changes. Add --apply --yes-i-understand only after reviewing the plan."); return; } if (!options.includes("--yes-i-understand")) throw new Error("Production mutation gate not satisfied. Pass --apply --yes-i-understand after human review."); if (report.summary.block) throw new BlockerError("Apply blocked by preflight. Resolve every blocker before requesting production operations."); output({ mode: "manual-only", report, plan: changePlan, applied: false, unsupported: true, exitCode: 3 }, json, "UNSUPPORTED: No production mutation was made. ShipLayer v0.1 has no write adapter; complete the listed manual steps in App Store Connect."); process.exitCode = 3; }
...
function validateOptions(command: string, options: string[]): void { const allowed: Record<string, Set<string>> = { init: new Set(["--force", "--json"]), analyze: new Set(["--json"]), prepare: new Set(["--out", "--json"]), check: new Set(["--json", "--remote"]), plan: new Set(["--json", "--remote"]), capture: new Set(["--json", "--execute", "--yes-execute"]), apply: new Set(["--json", "--apply", "--yes-i-understand"]), submit: new Set(["--json", "--submit", "--yes-submit"]) }; const known = allowed[command]; if (!known) return; for (let index = 0; index < options.length; index++) { const option = options[index]; if (!known.has(option)) { if (command === "prepare" && index && options[index - 1] === "--out") continue; throw new Error(`Unknown option '${option}' for ${command}.`); } if (option === "--out") { const value = options[++index]; if (!value || value.startsWith("--")) throw new Error("--out requires a relative output directory."); } } }
