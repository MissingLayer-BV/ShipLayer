...
export function assessNotCollectionEvidence(manifest: ShipLayerManifest, analysis: AnalysisReport, processor: ExternalProcessor): NotCollectionEvidenceAssessment {
  ...
  const relevant = relevantRuntimeEndpointFindings(manifest, analysis, processor, policyHost);
  ...
  for (const finding of relevant) {
    ...
    if (decisions.some((decision) => decision.disposition === "not-an-external-processor")) return blocked("a relevant runtime endpoint is human-confirmed as not an external processor", "Resolve the contradictory endpoint disposition. A runtime/API endpoint used to link this declared processor cannot also be confirmed as not an external processor; policy/docs-only links on a different non-runtime path may remain non-processor dispositions.", relevantFindingIds);
    if (!decisions.some((decision) => isExactDeclaredProcessorDecision(decision, finding, processor))) return blocked("a relevant runtime endpoint is not reconciled with an exact declared-processor decision and matching processor evidence", "For each scanner runtime/API endpoint on this processor's exact host (or explicitly mapped to this display-name processor), add one confirmed declared-processor externalServiceDecision with processorName, and use the same existing source evidence path in both the decision and externalProcessors[].evidence. Otherwise leave this row pending.", relevantFindingIds);
  }
  ...
  if (relevant.length) return { remediation: "", verified: "a scanner runtime endpoint and an exact declared-processor decision link the canonical policy hostname to this display-name processor; ShipLayer did not read or verify policy content", relevantFindingIds };
...
function relevantRuntimeEndpointFindings(manifest: ShipLayerManifest, analysis: AnalysisReport, processor: ExternalProcessor, policyHost: string) {
  const structuredHost = normalizedSafeHost(processor.name);
  return externalServiceFindings(analysis).filter((finding) => {
    if (!isRuntimeEndpointFinding(finding)) return false;
    const url = endpointUrl(finding);
    const endpointHost = url ? normalizedSafeHost(url.hostname) : undefined;
    if (endpointHost && endpointHost === structuredHost) return true;
