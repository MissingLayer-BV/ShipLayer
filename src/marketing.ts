...
\`\`\`
npm install
npx playwright install chromium   # one-time browser download; skip if already installed
npm run export
\`\`\`
...
If the bundled Chromium download is blocked on your machine, set \`SHIPLAYER_PW_CHANNEL=chrome\`
(or another installed Playwright channel) before running \`npm run export\` to use a
system-installed browser instead.
