...
// silently turns e.g. a fully transparent pixel into an opaque, wrong-colored one. The BG_R/BG_G/
// BG_B literals below are interpolated FROM SLIDE_BACKGROUND_RGB at module-load time (not
// hand-copied), so this text and the .canvas CSS background in renderSlideHtml genuinely cannot
// drift apart — both come from the one constant.
import { inflateSync, deflateSync } from "node:zlib";

const BG_R = ${SLIDE_BACKGROUND_RGB[0]};
const BG_G = ${SLIDE_BACKGROUND_RGB[1]};
const BG_B = ${SLIDE_BACKGROUND_RGB[2]};
