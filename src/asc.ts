...
function baseOperations(manifest: ShipLayerManifest): AscOperation[] { const operations: AscOperation[] = [
  ...
  { id: "manual.record-and-metadata", action: "manual", resource: "App record, metadata, availability, release", description: "Create/verify the app record, enter metadata/URLs/contact, select category/territories/release mode, confirm Apple Silicon Mac/Vision Pro availability, and select the uploaded build/version in App Store Connect. v0.1 does not write these fields.", safety: "manual", status: "unsupported" },
