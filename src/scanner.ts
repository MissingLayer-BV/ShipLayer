...
const NETWORK_IMPORTS = ["URLSession", "Alamofire", "Moya", "Firebase", "Sentry", "RevenueCat", "StoreKit", "UserNotifications", "Photos", "AVFoundation", "CoreLocation", "Contacts"];
...
export async function analyzeRepository(repository: string): Promise<AnalysisReport> {
  ...
  const push = (key: string, value: string, evidence: Evidence): void => { const normalized = value.trim().replace(/^["']|["']$/g, ""); const detectedKey = key === "bundleId" && /\.(?:tests|uitests)$/.test(normalized) ? "testBundleId" : key; (settings[detectedKey] ||= []).push({ value: normalized, evidence }); };
  const scanText = async (file: string): Promise<void> => {
    ...
    if (/\.(swift|m|mm|h)$/.test(file)) {
      for (const api of NETWORK_IMPORTS) if (new RegExp(`\\b(?:import\\s+${api}|${api})\\b`).test(content)) push(`api:${api}`, api, { source, excerpt: api, confidence: "medium", kind: "source-heuristic" });
  ...
  for (const file of walked.files.filter((file) => /(?:project\.yml|project\.pbxproj|Info\.plist|\.entitlements|PrivacyInfo\.xcprivacy|\.storekit|\.swift|\.m|\.mm|\.h)$/.test(file))) await scanText(file);
  ...
  return { schemaVersion: 1, repository: root, scannedAt: new Date().toISOString(), project: { xcodeProjects: xcodeProjects.sort(), workspaces: workspaces.sort(), projectYml: projectYml.map((file) => relative(root, file)).sort() }, findings: findings.sort((a, b) => a.key.localeCompare(b.key)), contradictions: contradictions.sort(), unresolvedQuestions: [...questions].sort(), ignored: { directories: walked.ignoredDirectories, filesOverLimit: walked.filesOverLimit, filesScanned: walked.files.length } };
}
