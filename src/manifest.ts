...
export function defaultManifest(input: Partial<ShipLayerManifest["app"]> = {}): ShipLayerManifest {
  const app = { name: input.name || "", bundleId: input.bundleId || "", deviceFamilies: input.deviceFamilies || ["iphone", "ipad"], locales: input.locales || ["en-US"], primaryLocale: input.primaryLocale || "en-US", availability: input.availability || "all", releaseKind: input.releaseKind || "needs-human-confirmation", releaseMode: input.releaseMode || "manual", ...input } as ShipLayerManifest["app"];
