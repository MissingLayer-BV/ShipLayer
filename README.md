...
Every non-consumable or subscription requires role-checked `purchasePresentation` evidence. Production paywall source must visibly render StoreKit `Product.displayPrice` (an unused read is insufficient), or use `ProductView`/`SubscriptionStoreView`; custom purchase UI must remain disabled/withheld while product/price is loading or unavailable. Conventional UI/unit/snapshot test source must assert both visible localized pricing and the unavailable purchase state. Hard-coded prices are blocked. Subscriptions additionally require source and test evidence for the visible billing period, applicable offer terms, and Terms/Privacy links before purchase.
